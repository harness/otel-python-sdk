...
def test_otlp_http_exporter_can_init(agent):
    exporter = agent._init.init_exporter(config_pb2.TraceReporterType.OTLP_HTTP)
    exporter.shutdown()
    assert isinstance(exporter, OTLPHttpSpanExporter)

def test_otlp_grpc_exporter_can_init(agent):
    exporter = agent._init.init_exporter(config_pb2.TraceReporterType.OTLP)
    exporter.shutdown()
...
def test_zipkin_exporter_can_init(agent):
    ...
    pass
    #exporter = agent._init.init_exporter(config_pb2.TraceReporterType.ZIPKIN)
    #exporter.shutdown()
