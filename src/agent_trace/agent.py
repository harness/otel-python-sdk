...
from agent_trace.env import get_env_value
...
from agent_trace.instrumentation.genai_env import maybe_set_genai_payload_capture_env_vars
from agent_trace.plugins.control import ControlPlugin, get_control_registry
from agent_trace.plugins.loader import load_control_plugins, load_observability_plugins
from agent_trace.version import __version__  # pylint:disable=C0413
...
class Agent:
    ...
    def __new__(cls):
        if cls._instance is None:
            with cls._singleton_lock:
                ...
                logger.debug('Agent Trace SDK version: %s', __version__)
    ...
    def __init__(self):
        ...
        logger.debug('Agent Trace SDK version: %s', __version__)
        logger.debug("successfully initialized agent trace sdk")
    ...
    def _instrument(self, library_key, app=None, auto_instrument=False):
        ...
        if library_key == DJANGO_KEY and auto_instrument is True:
            from agent_trace.instrumentation.django.django_auto_instrumentation_compat import (  # pylint: disable=C0415
        ...
        if library_key == FAST_API_KEY:
            from agent_trace.instrumentation.fast_api.fast_api_auto_instrumentation_compat import (  # pylint: disable=C0415
