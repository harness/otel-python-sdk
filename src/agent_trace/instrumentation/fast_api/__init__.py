...
async def replaced_ot_middleware_call(self, scope, receive, send):  # pylint:disable=R0914,R0915
    ...
    try:
        with self.tracer.start_as_current_span(
            ...
            if callable(self.server_request_hook):
                # OpenTelemetryMiddleware wraps server_request_hook in failsafe(), which always
            ...
            if should_forward_to_handler:
                ...
                code = int(control_result.response_status_code or 403)
                set_status_code(span, code)
                msg = control_result.response_message or 'Forbidden'
                if not isinstance(msg, (str, bytes)):
                    msg = str(msg)
                body_bytes = msg.encode('utf-8') if isinstance(msg, str) else bytes(msg)
                await send({
...
class TraceableException(HTTPException):
    def __init__(self, message='Forbidden', status_code=403, headers=None):
...
class FastAPIInstrumentorWrapper(BaseInstrumentorWrapper):
    ...
    def instrument(self):
        ...
        async def catch_exceptions_middleware(request: Request, call_next):
            ...
            except TraceableException as traceable_exception:
                return PlainTextResponse(status_code=traceable_exception.status_code,
                                         content=traceable_exception.message)
        ...
        # fast api handler(which results in 500 instead of whichever status code is configured)
    ...
    def client_response_hook(self, span, resp_data):
        ...
        if resp_phase == "http.response.start":
            ...
            if should_capture:
                body_data = resp_data.get('body') or b''
                # ASGI often ends with an empty body chunk (more_body=False); do not wipe a prior capture.
                if isinstance(body_data, (bytes, bytearray)) and len(body_data) == 0:
                    return
