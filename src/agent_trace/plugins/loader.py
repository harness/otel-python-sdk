...
CONTROL_ENTRY_GROUP = "agent_trace_control_plugin"
OBSERVABILITY_ENTRY_GROUP = "agent_trace_observability_plugin"
    ...
    for name in _ordered_plugin_names(config, "control"):
        ...
        if entry_point is None:
            logger.warning(
                ...
                "(pip install the package that provides agent_trace_control_plugin)",
                name,
            )
            continue
        try:
    ...
    for name in _ordered_plugin_names(config, "observability"):
        ...
        if entry_point is None:
            logger.warning(
                ...
                "(pip install the package that provides agent_trace_observability_plugin)",
                name,
            )
            continue
        try:
